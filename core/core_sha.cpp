...
inline static uint32 f1(uint32 e, uint32 f, uint32 g) {
    uint32 term1 = rotate(e, 6) ^ rotate(e, 11) ^ rotate(e, 25);
    uint32 term2 = (e & f) ^ (~e & g); //(g ^ (e & (f ^ g)))
    return term1 + term2;
}

inline static uint32 f2(uint32 a, uint32 b, uint32 c) {
    uint32 term1 = rotate(a, 2) ^ rotate(a, 13) ^ rotate(a, 22);
    uint32 term2 = ((a | b) & c) | (a & b); //(a & (b ^ c)) ^ (b & c);
    return term1 + term2;
}

// process 64 bytes
void SHA256::_process_block(const void* data) {
    // get last hash
    uint32 a = hash_[0];
    uint32 b = hash_[1];
    uint32 c = hash_[2];
    ...
    const uint32* input = (uint32*) data;
...
void SHA256::get_hash(unsigned char buffer[SHA256::HashBytes]) {
