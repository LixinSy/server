...
static void ntyWorkerCleanup(void * pool_) {
    nThreadPool *pool = static_cast<nThreadPool*>(pool_);
    --pool->nthreads;
...
static void ntyJobCleanup(void *pool_) {
    nThreadPool *pool = static_cast<nThreadPool*>(pool_);
...
void pthread_mutex_lock_clean_up(void *mutex_)
{
...
void nThreadPoolWait(nThreadPool *pool) {
    ...
    pthread_cleanup_push(pthread_mutex_lock_clean_up, &pool->mtx);
...
void hello(void *arg) {
    ...
    int index = *(int*)arg;
    printf("hello  hello index : %d, selfid : %lu\n", index, pthread_self());
    free(arg);
    usleep(1);
}
...
void  test_pthread() {
    ...
    nThreadPool *pool = ntyThreadPoolCreate(5, 10, 15, NULL);
    ...
    for (i = 0;i < KING_COUNTER_SIZE;i ++) {
        ...
        //else
        //    ntyThreadPoolQueue(pool, hello, index);
    }
